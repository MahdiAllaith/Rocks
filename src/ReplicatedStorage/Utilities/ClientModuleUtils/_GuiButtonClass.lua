--!strict
--@author: crusherfire & CoderActual - Adjust by @IronBeliever _ Added Handler for imageColor 
--@date: 8/9/24
--[[@description:
...
export type ButtonTweenInfo = {
	...
	ImageColor3: TweenInfo?, -- Add this line
	Size: TweenInfo?,
	Position: TweenInfo?,
	Rotation: TweenInfo?
}
...
type TweenableProperties = {
	...
	ImageColor3: Color3?, -- Add this line
	Size: UDim2?,
	Position: UDim2?,
	Rotation: number?,
}
...
export type ButtonTheme = {
	...
	Callbacks: {
		Enter: ((self: WrappedGuiButton) -> ())?,
		Exit: ((self: WrappedGuiButton) -> ())?,
	}?
}
...
-- Replace your existing theme application functions with these fixed versions:
...
--	do
--		-- cancel any tweens!
--		local names = { "HoverColor", "HoverSize", "HoverPosition", "HoverRotation", "HoverImageColor" }
...
--			local goal = {}
--			if name == "BackgroundColor3" or name == "ImageColor3" then
--				goal[name] = value
--			else
...
--	if hoverTheme.Callbacks and hoverTheme.Callbacks.Enter then
--		hoverTheme.Callbacks.Enter(self)
--	end
--end
...
--			local goal = {}
--			goal[name] = value -- For exit, we always use the direct value

--			local tween = TweenService:Create(self._button, exitTweenInfo[name], goal)
--			self._activeTweens.Exit[`Hover{name}`] = tween
--			tween:Play()
--		else
--			local button = self._button :: any -- resolves a type annotation issue
--			button[name] = value
...
--	if hoverTheme.Callbacks and hoverTheme.Callbacks.Exit then
--		hoverTheme.Callbacks.Exit(self)
--	end
--end
...
function applyInputEnterTheme(self: WrappedGuiButton)
	...
	for name, value in pairs(hoverTheme.Properties.Enter) do
		...
		-- Apply hover even if selected
		local tweenInfo = enterTweenInfo[name]
		if tweenInfo then
			local goal = {}
			goal[name] = finalValue
			local tween = TweenService:Create(self._button, tweenInfo, goal)
			self._activeTweens.Enter[`Hover{name}`] = tween
			tween:Play()
		else
			self._button[name] = finalValue
		end
	end
...
function applyMouseLeaveTheme(self: WrappedGuiButton)
	...
	for name, value in pairs(hoverTheme.Properties.Exit) do
		...
		-- If selected, override with selection theme Exit for this property
		if self:IsSelected() and selectionTheme.Properties.Enter[name] then
			finalValue = selectionTheme.Properties.Enter[name]
		end

		local tweenInfo = exitTweenInfo[name]
		if tweenInfo then
			local goal = {}
			goal[name] = finalValue
			local tween = TweenService:Create(self._button, tweenInfo, goal)
...
--	do
--		-- cancel any tweens!
--		local names = { "SelectionColor", "SelectionSize", "SelectionPosition", "SelectionRotation", "SelectionImageColor" }
...
function applySelectedTheme(self: WrappedGuiButton)
	...
	-- 🔹 Call the selection Enter callback
	if selectionTheme.Callbacks and selectionTheme.Callbacks.Enter then
		selectionTheme.Callbacks.Enter(self)
	end
end
...
--			local tween = TweenService:Create(self._button, exitTweenInfo[name], goal)
--			self._activeTweens.Exit[`Selection{name}`] = tween
--			tween:Play()
--		else
--			local button = self._button :: any -- resolves a type annotation issue
--			button[name] = value
...
function applyDeselectedTheme(self: WrappedGuiButton)
	...
	-- 🔹 Call the selection Exit callback
	if selectionTheme.Callbacks and selectionTheme.Callbacks.Exit then
		selectionTheme.Callbacks.Exit(self)
	end
end
...
function MT.ModifyTheme(self: WrappedGuiButton, which: "Hover" | "Selection", themeModification: ButtonThemeModification): WrappedGuiButton
	...
	-- Ensure ExitTweenInfo exists for everything Enter has
	...
	-- Ensure Exit properties are filled
	...
	if themeModification.Callbacks then
		newTheme.Callbacks = newTheme.Callbacks or {}
		for key, fn in pairs(themeModification.Callbacks) do
			newTheme.Callbacks[key] = fn
		end
	end
