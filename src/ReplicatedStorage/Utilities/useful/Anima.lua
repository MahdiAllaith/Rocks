...
type config = {fadeTime: number?,weight: number?,priority: Enum.AnimationPriority?,stopOthers: boolean?,loop: boolean?}
...
--- @return Anima
--- @example
--- local anima = Anima.new(workspace.Animations, game.Players.Player1, true) -- Enable debug mode
function handler.new(animationsFolder: Folder, player: Player,debugMode: boolean?)
...
-- @param enabled boolean Wheter debug mode should be enabled
...
function handler:Cache()
	...
	for name, oldData in pairs(oldTracks) do
		if oldData.track then
			...
			if oldData.markerConnections then
				for _, conn in pairs(oldData.markerConnections) do
					pcall(function()
						conn:Disconnect()
					end)
				end
	...
	for name, animation in pairs(self.animations) do
		...
		if markerCallbackMap then
			...
			if hasGetMarkerSignal then
				for markerName, callback in pairs(markerCallbackMap) do
	...
	self._stats.tracksCreated = (self._stats.tracksCrcccccccceated or 0) + created
...
function handler:WatchForChanges()
	...
	local function watchAnimationProperties()
		for _, animation in pairs(self.animationsFolder:GetChildren()) do
			if animation:IsA("Animation") then
...
-- Fix 1: Update the PlayAnimation function to use cached tracks properly
